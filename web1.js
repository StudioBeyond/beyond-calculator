...
const value = {
  ...
  pageValuePerTemplate: 0,

  pageValueTotalUnique: 0,
  pageValueTotalTemplate: 0,
  pageValueTotalAll: 0,
...
const pageTemplateInput = document.querySelector(`[${webcalcAttribute}="page-template-input"]`);
const pageTemplateDisplay = document.querySelector(`[${webcalcAttribute}="page-template-display"]`);
const pageInputs = Array.from([pageUniqueInput, pageTemplateInput]);
value.pageValuePerUnique = parseFloat(pageUniqueInput
  .getAttribute(valueAttribute))
value.pageValuePerTemplate = parseFloat(pageTemplateInput
  .getAttribute(valueAttribute))
...
function calculatePages() {
  ...
  value.pageCountTemplate = parseFloat(pageTemplateInput.value);
  value.pageCountTotal = parseFloat(value.pageCountUnique + value.pageCountTemplate);

  value.pageValueTotalUnique =
    value.pageValuePerUnique *
    value.pageCountUnique
  value.pageValueTotalTemplate =
    value.pageValuePerTemplate *
    value.pageCountTemplate
...
pageInputs.forEach(input => {
  input.addEventListener('input', () => {
    ...
    updateSliderDisplay(pageTemplateInput, pageTemplateDisplay)
    calculateTotal();
  });
});
